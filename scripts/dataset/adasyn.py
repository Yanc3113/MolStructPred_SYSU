...
input_dir = "dataset"
output_dir = "dataset_adasyn"
os.makedirs(output_dir, exist_ok=True)

splits = ["train"]

for split_name in splits:
    print(f"Processing {split_name} set for ADASYN oversampling...")
    ...
    ada = ADASYN(random_state=66,sampling_strategy=1.0)
    X_resampled, y_resampled = ada.fit_resample(X, y)

    print(f"New {split_name} distribution after ADASYN: {Counter(y_resampled)}")
    ...
    out_csv = os.path.join(output_dir, f"{split_name}_fingerprints_adasyn.csv")
    ...
    print(f"Saved ADASYN {split_name}: {out_csv} (graphs kept as original {out_pt})")

print("ADASYN oversampling done for training set.")
